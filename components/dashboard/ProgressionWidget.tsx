...
function Delta({ base, current }: { base: number; current: number }) {
  ...
  if (diff === 0) return <span style={{ fontSize: '.68rem', color: 'var(--t3)' }}>±0</span>
  const sign = diff > 0 ? '+' : ''
...
export function ProgressionWidget({ initialBaselines }: { initialBaselines: Baseline[] }) {
  ...
  if (baselines.length === 0) {
    return (
      <div style={{ background: 'var(--card)', border: '1px solid var(--b1)', borderRadius: '12px', padding: '1.1rem 1.25rem' }}>
        <div style={{ fontSize: '.8rem', fontWeight: 600, color: 'var(--t1)', marginBottom: '.3rem' }}>Progression abonnés</div>
  ...
  return (
    <div style={{ background: 'var(--card)', border: '1px solid var(--b1)', borderRadius: '12px', padding: '1.1rem 1.25rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '.85rem' }}>
        <span style={{ fontSize: '.8rem', fontWeight: 600, color: 'var(--t1)' }}>Progression abonnés</span>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '.6rem' }}>
        {baselines.map(b => (
          <div key={b.platform} style={{ display: 'flex', alignItems: 'center', gap: '.6rem' }}>
            <div style={{ width: '28px', height: '28px', borderRadius: '8px', background: 'var(--s2)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <PlatformIcon platform={b.platform} />
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '.35rem' }}>
                <span style={{ fontSize: '.9rem', fontWeight: 700, color: 'var(--t1)', lineHeight: 1 }}>
